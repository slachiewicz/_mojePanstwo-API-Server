...
if( $osoba_id = $this->DB->selectValue("SELECT krs_id FROM krs_kandydaci WHERE kandydat_id='$id' AND correct='3' LIMIT 1") ) {

	$osoba_id = addslashes($osoba_id);
	
	$osoba = $this->DB->selectAssoc("SELECT liczba_reprezentanci, liczba_wspolnicy, liczba_nadzorcow, liczba_akcjonariusze, liczba_zalozyciele, liczba_prokurenci FROM krs_osoby WHERE id='$osoba_id'");
	...
	`organizacja`.`id`, `organizacja`.`nazwa`, `organizacja`.`forma_prawna_str`, `organizacja`.`data_rejestracji`, `organizacja`.`kapital_zakladowy`, `organizacja`.`wykreslony`, `organizacja`.`adres_miejscowosc` 
	FROM `krs_reprezentanci` as `rola` 
	JOIN `krs_pozycje` as `organizacja` 
	ON `rola`.`pozycja_id` = `organizacja`.`id`
	WHERE `rola`.`osoba_id` = '$osoba_id' 
	AND `rola`.`deleted` = '0' 
