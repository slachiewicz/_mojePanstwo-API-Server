...
class OAuthController extends OAuthAppController
{
    ...
    public function login()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Auth->login()) {
                ...
                $user = $this->User->checkAndLoginAgainstPostImport($data, $this->Auth->password($this->data['User']['password']));
                if ($user) {
                    $this->Auth->login($user);
                    $this->_log(array('msg' => 'LC_LOG_LOGIN', 'ip' => $this->request->clientIp(), 'user_agent' => env('HTTP_USER_AGENT')));
    ...
    public function userinfo()
    {
        ...
        $this->loadModel('User');
        $user = $this->User->find('first', array('conditions' => array('User.id' => $user['id']), 'fields' => array('User.id', 'User.email')));
        $user = $user['User'];
        unset($user['logged_before']);
        unset($user['personal_gender']);
        $user['name'] = $user['personal_name'];
        unset($user['personal_name']);
        $user['lastname'] = $user['personal_lastname'];
        unset($user['personal_lastname']);
    ...
    public function add()
    {
        var_dump($this->OAuth->Client->add('http://mp.localhost'));
    }
