...
class DboSource extends DataSource
{
    ...
    protected function _parseKey($model, $key, $value)
    {
        ...
        # niestety musialem to zrobić, problem leży w interpretacji liczbowych enumów
        if (is_object($model)) {
            if (preg_match('/enum/', $model->getColumnType($key)) && preg_match('/(^[\d]+$)/', $value)) {
                return "{$key} {$operator} \"{$this->value($value)}\"";
            }
            return "{$key} {$operator} {$value}";
        } else {
